...
import { SalesQuotationAPI } from '../../api/url'; // Make sure to define this endpoint
...
export const fetchAllSalesQuotations = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get(SalesQuotationAPI);
...
export const deleteSalesQuotation = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      await axios.delete(`${SalesQuotationAPI}/${id}`);
      return id;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.response.data);
    }
...
export const { convertSalesQuotation, viewSalesQuotation, editSalesQuotation } = salesQuotationSlice.actions;
