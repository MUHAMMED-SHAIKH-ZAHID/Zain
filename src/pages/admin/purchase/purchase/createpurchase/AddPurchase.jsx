...
const purchaseValidationSchema = Yup.object({
  ...
  paid_amount: Yup.number().min(0, 'Paid amount must be at least 0')
  .max(Yup.ref('grand_total'), 'Paid amount cannot exceed grand total'),
...
const AddPurchase = () => {
    ...
    onSubmit: (values) => {
      ...
      promise.then((res) => {
        ...
        if (res.payload.errors){
          setLoadingMessage(false)
          toast.error(res.payload.errors.invoice_number[0])
        }
      });
    },
  });
  ...
  const importPurchaseOrder = () => {
    setCustomErrMsg('')
    if (formik.values.purchase_order_number) {
      ...
      if (ReferensePurchase == undefined) {
        ...
        setImportedvendor(ReferensePurchase?.supplier_id)
        formik.setFieldValue('supplier_id',ReferensePurchase?.supplier_id)
   ...
   console.log(suppliers,"the suppliers")
      ...
      <form onSubmit={formik.handleSubmit} className="space-y-5">
          ...
          <div className="flex w-full gap-4">
            <div className='w-full'>
              ...
              <Select
                  ...
                  value={suppliers.find(item=> item.id == formik.values.supplier_id)?.company_name}
            ...
            <div className='w-full'>
              ...
              <input
                ...
                className=" block w-full px-4 py-[5px] border border-gray-300  rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
              />
        ...
        <div className="mt-10">
          <div className="mt-4">
            <div className="mb-2 flex justify-end">
              <button type="button" onClick={() => setShowModal(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-medium leading-none py-2 px-4 rounded">
                Add Products
              </button>
            </div>
                    ...
                    <td className="px-6  whitespace-nowrap text-sm text-gray-500">{item?.outbound || 0}</td> */}
                      ...
                      <td className="px-6  whitespace-nowrap flex items-center gap-4 justify-center text-right text-sm font-medium">
                        <div className="flex items-center ">
                          <button onClick={() => handleEditItem(index, item.product_id)} className="text-primary-600 hover:text-red-900">
        ...
        <div className="flex gap-4 justify-end mt-5">
          ...
          <div>
            ...
            <input
              id="tax_amount"
              type="text"
              value={parseInt(taxAmount).toFixed(2)}
              disabled
              className="mt-1 block w-full px-4 py-2 border border-gray-300 bg-gray-50 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            />
          </div>
        ...
        <div className="flex justify-center my-4" >
          <button disabled={loadingMessage} type="submit" className={`inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${loadingMessage ? "animate-pulse" : ''}`} >
            {loadingMessage ? 'Submiting Purchase...' : 'Submit Purchase '}
          </button>
        </div>
      </form>
