...
const EditItemsForm = ({ items, setItems, onClose }) => {
  const {  products,editsalescolumn,editsalesindex,loading, error } = useSelector((state) => state?.sales);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      quantity: editsalescolumn?.quantity,
      inbound: 0,
      outbound: 0,
      price: editsalescolumn?.price,
      tax: editsalescolumn?.tax,
      total: editsalescolumn?.total,
      tax_amount: editsalescolumn?.tax_amount,
      total_inc_tax: editsalescolumn?.total_inc_tax,
      id:editsalescolumn?.id,
    },
    validationSchema: itemSchema,
    onSubmit: (values, { resetForm }) => {
        console.log(values,"checking the items to update value",editsalescolumn.id,items[0].id)
      ...
      console.log(updatedItems,"checking the items to update",editsalesindex)
