...
const EditSales = () => {
  ...
  const navigate = useNavigate()
    const { customer,editsales,products,paymentModes, loading, error } = useSelector((state) => state?.sales);

     console.log(customer,"customer list");
    ...
    console.log(editsales,"checking editsalesid",editsales?.id)
    ...
    const formik = useFormik({
        initialValues: {
            sales_date: editsales?.sales_date || new Date(), // Default to current date if undefined
            sales_number: editsales?.sales_number || '', // Using sales_number instead of purchase_number
            customer_id: editsales?.customer_id || '', // Using customer_id instead of supplier_id
            payment_status: editsales?.payment_status || '',
            paid_amount: editsales?.paid_amount || 0,
            payment_balance: editsales?.payment_balance || 0,
            payment_due_date: editsales?.payment_due_date || null, // Default to null if undefined
            discount: editsales?.discount || 0,
            account_id: editsales?.account_id || '', // Default to 'cash' if undefined
            sales_items: editsales?.sales_items || [], // Using sales_items instead of purchase_items
            notes: editsales?.notes || '',
            total: editsales?.total || 0,
            grand_total: editsales?.grand_total || 0,
            tax_amount: editsales?.tax_amount || 0,
        },
        validationSchema: purchaseValidationSchema,
        onSubmit: (values) => {
            console.log("testing ddddddd");
            console.log('Final Submission:', values);
            dispatch(updateSale({ id: editsales.id, saleData: values }))
            navigate('/sales/')
  ...
  return (
    <div className="  p-5">
      <h2 className="text-xl font-bold mb-5 text-center">Edit Sales</h2>
      ...
      <div className="flex justify-between">
        ...
        <div>
            <label htmlFor="sales_number" className="block text-sm font-medium text-gray-700">Customer ID:</label>
            <input
            disabled
                id="sales_number"
                type="text"
