...
useEffect
  const dispatch = useDispatch()
    const { purchaseOrders,paymentModes, loading, error } = useSelector((state) => state?.purchases);
    const { customers,products } = useSelector((state) => state?.creditNotes);
...
<div className="mb-2 flex justify-end">
  {formik.values.customer_id ?  <button type="button" onClick={() => setShowModal(true)} className="bg-blue-500 hover:bg-blue-700 text-white font-medium leading-none py-2 px-4 rounded">
          Add Products
 ...
 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item?.reason}</td>
  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item?.sales_invoice_no}</td>
...
<div className=""> &nbsp; ₹ {formik?.values?.grand_total}</div>
        ...
        >
         {loadingMessage ? 'Submiting Credit Note...': 'Submit Credit Note' }
        </button></div>
        </form>
