...
import { PaymentApprove, createPayment } from '../../../redux/features/PaymentSlice';
...
const ApprovePayment = ({ show, handleClose, data = {} }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const promise = dispatch(PaymentApprove({id:data?.id}));
        promise.then((res) => {
...
const modalContent = (
    ...
    <div className='w-full flex items-center'>
      ...
      <div className='flex w-full'>
        <label htmlFor="person_id" className="block text-sm w-40 font-medium text-gray-700"> {formik.values.reference_type === 'bill' ? 'Vendor' : 'Customer'   } Name</label>
        <select
        disabled
...
{/* 
  ...
  return (
    <Modal2 visible={show} onClose={handleClose} id="view-Payment-modal" content={modalContent} title=" Payment process" size='lg' />
