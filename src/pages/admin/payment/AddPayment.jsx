...
const AddPayment = ({ handleClose }) => {
  ...
  const { customers, vendors,accounts } = useSelector((state) => state.payments);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        formik.values.due_amount = (parseFloat(formik.values.due_amount) -  parseFloat(formik.values.paid_amount)).toFixed(2)
        formik.values.oldpaid_amount = (parseFloat(formik.values.paid_amount) + parseFloat(formik.values.oldpaid_amount)).toFixed(2)
        formik.values.paid_amount = (parseFloat(formik.values.paid_amount)).toFixed(2)
  ...
  <option value={item.id} key={item.id}>
    {item.account_name}
      ...
      <div className="flex justify-end space-x-2 mt-5">
        ...
        >
         {loadingMessage ? 'Adding Payment...': 'Add Payment' }
        </button>
        </div>
    </form>
  );
