...
const ViewInvoice = () => {
  ...
  const navigate = useNavigate()
    const { customers,viewsalesdata,products,paymentModes, loading, error } = useSelector((state) => state?.executivesales);
    ...
    const formik = useFormik({
      ...
      const {  saleorder } = useSelector((state) => state?.executivesales);
      const [selectedShippingAddress,setSelectedShippingAddress]=useState([])
      const [filteredProducts,setFilteredProducts] = useState([])
      const matchingPurchaseOrder =  saleorder?.find((po) => formik.values.sales_order_id == po.id);
      useEffect(() => {
        formik.setFieldValue('customer_id',viewsalesdata?.customer_id)
