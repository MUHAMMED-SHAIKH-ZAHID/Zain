...
import { updateSalesQuotation } from '../../../redux/features/SalesQuotationSlice';
...
const purchaseValidationSchema = Yup.object({
  ...
  sale_order_number:Yup.string()
}); 

const EditSalesQuotation = () => {
  const dispatch = useDispatch()
  const navigate= useNavigate()
    const { customers,editSales,products, loading, error } = useSelector((state) => state?.executiveSalesOrder);
    ...
    const formik = useFormik({
      initialValues: {
        ...
        expected_delivery_date:  editSales?.expected_delivery_date || '',
        payment_terms:  editSales?.payment_terms || '',
      ...
      onSubmit: (values) => {
        ...
        const promise =dispatch(updateSalesQuotation({ id: editSales.id, salesData: values }))
  ...
  return (
    ...
    <h2 className="text-xl font-medium mb-5 text-center">Edit Invoice Order</h2>
    <form onSubmit={formik.handleSubmit} className="space-y-4">
      <div className="grid grid-cols-3 gap-5">
        <div>
          ...
          <Select
                  ...
                  value={customers.find(item => item.id == editSales.customer_id)?.name}
      ...
      <button
        ...
        >
         {loadingMessage ? 'Updating  Invoice Order...': 'Update Invoice  Order' }
        </button>
      </div>
    </form>
