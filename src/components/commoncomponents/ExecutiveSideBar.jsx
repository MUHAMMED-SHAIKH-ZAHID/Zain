...
const SidebarData = [
  { name: "Dashboard", icon: MdDashboard, path: "/executive/dashboard" },
  { name: "Customer", icon: BsPersonBoundingBox, path: "/executive/customers" },


  { name: "Payment", icon: MdOutlinePayment, path: "/executive/payment/" },
  { name: "Invoice Order", icon: BsBlockquoteRight, path: "/executive/invoice/order" },
  { name: "Invoice ", icon: FaFileInvoice, path: "/executive/invoice" },
  { name: "Stock", icon: MdOutlineProductionQuantityLimits, path: "/executive/stock" },
  ...
  return (
    <div className={`h-screen transition-all duration-300 ${isOpen ? 'w-52' : 'w-20'} bg-gray-900`}>
      <div className="flex flex-col mx-4 h-full backdrop-blur-lg shadow-md">
        ...
        {isOpen && <div className="text-[.6rem] text-center mb-3 text-gray-400">Executive Management Dashboard</div>}
